...
class VOCDataset(Dataset):
    ...
    def __init__(self, split, size, data_dir='/home/arpitsah/Desktop/Fall-2023/VLR/hw1/q1_q2_classification/data/VOCdevkit/VOC2007/'):
        super().__init__()
        self.split = split
        ...
        for index in self.index_list:
            ...
            for child in root:
                if child.tag =='object':
                    for item in child:
                        if item.tag == 'name':
                            idx = self.get_class_index(item.text)
                            class_vec[idx] = 1
                        if item.tag == 'difficult':
                            weight_vec[idx] = 0 if item.text == '1' else 1
