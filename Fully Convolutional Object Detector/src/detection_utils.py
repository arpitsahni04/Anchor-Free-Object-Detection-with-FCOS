...
def fcos_get_deltas_from_locations(
    ...
    deltas[:, 0] = (locations[:, 0] - gt_boxes[:, 0]) *(1 /stride)
    deltas[:, 1] = (locations[:, 1] - gt_boxes[:, 1]) *(1 /stride)
    deltas[:, 2] = (gt_boxes[:, 2] - locations[:, 0]) *(1 /stride)
    deltas[:, 3] = (gt_boxes[:, 3] - locations[:, 1] ) *(1 /stride)
    mask = gt_boxes[:,:4].sum(dim=1)
    mask = (mask == -4)
    deltas[mask] = -1
    ##########################################################################
    #                             END OF YOUR CODE                           #
...
def fcos_apply_deltas_to_locations(
    ...
    deltas = stride * torch.clip(deltas, min = 0)
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    # )
    ##########################################################################
    centerness = torch.zeros((deltas.shape[0], ))
    centerness = centerness.cuda()
    L= deltas[:,0]
    T= deltas[:,1]
    R= deltas[:,2]
    B= deltas[:,3]
    centerness = torch.div((torch.min(L, R) * torch.min(T, B)),(torch.max(L, R) * torch.max(T, B)))
    centerness = torch.sqrt(centerness)    
    # centerness = torch.clamp(centerness, 0, 1)
    mask = (L == -1)
    centerness[mask]=-1
    ##########################################################################
    #                             END OF YOUR CODE                           #
...
def get_fpn_location_coords(
    ...
    for level_name, feat_shape in shape_per_fpn_level.items():
        ...
        ######################################################################
        y_dim,x_dim = feat_shape[2],feat_shape[3]
        
        xc = torch.arange(0,x_dim,dtype=dtype, device=device)
        yc = torch.arange(0,y_dim,dtype=dtype, device=device) 
        
        gridy,gridx = torch.meshgrid(yc,xc)
        gridx = (gridx.float() + 0.5) 
        gridy = (gridy.float() + 0.5) 
        gridx*=level_stride
        gridy*=level_stride
        level  = torch.stack((gridy,gridx),dim=-1)
        level = level.view(-1,2).cuda()
        location_coords[level_name] = level
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
